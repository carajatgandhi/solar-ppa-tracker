...
def detect_entry_type(text):
    ...
    # PPA Signed — company signs/inks a PPA with someone
    signed_keywords = [
        "signs ppa", "sign ppa", "signs ppas", "signed ppa",
        "inks ppa", "ink ppa", "signs power purchase", "signed power purchase",
        "power purchase agreement signed", "ppa signed", "ppa with",
        "signs solar ppa", "signs open-access", "signs open access",
        "long-term ppa", "long term ppa", "secures ppa", "signs renewable",
        "signs energy", "agreement with", "supply agreement",
    ]

    # Tender Allotted — someone wins/gets awarded a project
    allotted_keywords = [
        "allot", "awarded", "wins bid", "win bid", "bags ",
        "secures bid", "wins tender", "win tender", "l1 bidder",
        "lowest bidder", "clinches", "gets order", "selected as developer",
        "wins ", "win ", "bags contract", "gets contract",
        "wins contract", "win contract", "wins order", "win order",
        "wins auction", "win auction", "wins project", "win project",
        "selected for", "empanelled",
    ]

    # Tender Issued — new tender floated
    issued_keywords = [
        "issues tender", "floats tender", "invites bid", "invites bids",
        "rfp issued", "issues rfq", "calls for bid", "new tender",
        "tender out", "seeks developer", "invited tender", "tenders for",
        "issues rfs", "floats rfs", "issues rfq",
    ]

    # Policy / Regulation
    policy_keywords = [
        "regulation", "policy", "amendment", "directive",
        "cerc", "uperc", "merc", "tnerc", "kerc",
        "approve tariff", "tariff order", "approves tariff",
        "benchmark tariff", "generic tariff",
    ]

    # Check signed FIRST — most specific for your use case
    if any(k in t for k in signed_keywords):
        return "PPA Signed"
    if any(k in t for k in allotted_keywords):
        return "Tender Allotted"
    if any(k in t for k in issued_keywords):
